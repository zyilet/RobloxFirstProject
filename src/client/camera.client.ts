...
player.CharacterAdded.Connect(c => {
    ...
    let humanoid = c.WaitForChild("Humanoid");
    ...
    let playerInput = function (actionName: string, inputState: Enum.UserInputState, inputObj: InputObject) {
        if (inputState === Enum.UserInputState.Change) {
            cameraAngleX -= inputObj.Delta.X;
            cameraAngleY = math.clamp(cameraAngleY - inputObj.Delta.Y * 0.5, -75, 75);
        }
    }
    let cameraUpdate = function (dt: number) {
        let startCFrame = new CFrame(rootPart.CFrame.Position).mul(CFrame.Angles(0, math.rad(cameraAngleX), 0));
